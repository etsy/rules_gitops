...
func executeBazelQuery(bazelCmd string, query string) *analysis.CqueryResult {
	log.Printf("Running Bazel Query: %s", query)
	cmd := osexec.Command(bazelCmd, "cquery",
		"--output=proto",
		"--noimplicit_deps",
		query)
...
func processTarget(target, bazelCmd string, bazelFlags []string) {
